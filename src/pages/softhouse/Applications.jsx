...
//   // View student profile (aligned with handleView, without achievements)
...
//   // Export to Excel (without achievements)
//   const exportToExcel = () => {
//     if (!viewData) return;
...
//   // Export to PDF (without achievements)
//   const exportToPDF = () => {
//     if (!viewData) return;
...
//                   <span
//                     style={{
//                       color: item.status === 1 ? 'green' : item.status === 2 ? '#dc3545' : 'orange',
//                       fontWeight: 'bold',
//                     }}
//                   >
...
//                     <>
//                       <Button
//                         onClick={() => toggleStatus(item._id, 1)}
//                         style={{
...
//                       </Button>
//                       <Button
//                         onClick={() => toggleStatus(item._id, 2)}
//                         style={{
...
const Applications = () => {
  ...
  const viewProfile = async (studentId, application) => {
    try {
      setLoadingProfile(true);
      const token = localStorage.getItem('token');
      if (!token) {
      ...
      // Combine the data and include resume from application
      const combinedData = {
        ...userResponse.data,
        ...profileResponse.data,
        ...studentResponse.data,
        resume: application.studentDetails?.resume || null, // Include resume object or null
      };
  ...
  // Download resume
  const downloadResume = async (resume, studentName) => {
    ...
    try {
      ...
      link.href = url;
      link.setAttribute('download', resume.fileName || `${studentName || 'student'}_resume.pdf`);
    ...
    const studentData = [
      {
        ...
        Skills: viewData.skill || 'N/A',
        Status: viewData.status === 1 ? 'Active' : 'Inactive',
        Resume: viewData.resume?.fileName || 'N/A',
      },
    ];
        ...
        <Table striped bordered hover responsive>
          ...
          <tbody>
            {data.map((item) => (
              <tr key={item._id}>
                <td>{item.internship?.title || 'N/A'}</td>
                <td>{item.studentDetails?.name || item.user?.name || 'N/A'}</td>
                <td>{item.studentDetails?.email || item.user?.email || 'N/A'}</td>
                <td>{item.internship?.type || 'N/A'}</td>
                <td>
                  <span
                    className={`font-bold ${
                ...
                <td>
                  {item.studentDetails?.resume?.filePath ? (
                    <button
                      onClick={() =>
                        downloadResume(
                          item.studentDetails.resume,
                          item.studentDetails?.name || item.user?.name
                        )
                      }
                      className="flex items-center gap-2 px-3 py-1 bg-blue-100 text-blue-700 rounded-lg hover:bg-blue-200 focus:outline-none focus:ring-2 focus:ring-blue-500 transition-colors"
                      aria-label={`Download resume for ${item.studentDetails?.name || item.user?.name || 'student'}`}
                    >
                      <FaDownload /> Download
                    </button>
                  ) : (
                ...
                <td className="space-x-2">
                      {item.status === 0 && (
                        <>
                          <Button
                            onClick={() => toggleStatus(item._id, 1)}
                            variant="success"  // Changed to use Bootstrap's success variant (green)
                            className="hover:bg-green-700 rounded px-3 py-1"
                          >
                            Approve
                          </Button>
                          <Button
                            onClick={() => toggleStatus(item._id, 2)}
                            variant="danger"  // Changed to use Bootstrap's danger variant (red)
                            className="hover:bg-red-700 rounded px-3 py-1"
                          >
                            Reject
                          </Button>
                        </>
                      ...
                      <Button
                        onClick={() => viewProfile(item.studentId, item)}
                        variant="primary"
                        className="hover:bg-blue-700 rounded px-3 py-1"
                        disabled={loadingProfile}
                      >
                        {loadingProfile ? 'Loading...' : 'View Profile'}
                      </Button>
                              ...
                              <div className="bg-blue-200 p-6 rounded-lg">
                                ...
                                <div className="space-y-4">
                                  ...
                                  {viewData.resume?.filePath && (
                                    <div className="flex items-center">
                                      <FaFilePdf className="text-blue-500 mr-2" />
                                      <button
                                        onClick={() =>
                                          downloadResume(viewData.resume, viewData.name)
                                        }
                                        className="text-sm text-blue- calamitous hover:underline focus:outline-none focus:ring-2 focus:ring-blue-500"
