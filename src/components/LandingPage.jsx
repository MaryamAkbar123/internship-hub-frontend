...
const LandingPage = () => {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setActiveInternship((prev) => (prev + 1) % (filteredInternships.length || 1));
    }, 5000);
    return () => clearInterval(interval);
  }, [filteredInternships.length]);
          ...
          <div className="container mx-auto px-4">
            ...
            >
              ...
              <FaSearch 
                className="absolute right-4 top-1/2 transform -translate-y-1/2 text-blue-500 cursor-pointer"
                onClick={() => {
                  const currentDate = new Date("2025-06-17T22:16:00+05:00");
                  const filtered = internships
                    .filter((internship) => {
                      if (!internship.deadline) return false;
                      const deadline = new Date(internship.deadline);
                    ...
                    {filteredInternships
                      ...
                      .map((internship, index) => (
                        ...
                        >
                          <div className="md:w-1/2 h-64 md:h-full">
                            <img
                              src={internship.image ? `/images/${internship.image}` : `/images/p1.jpeg`}
                              alt={internship.title}
                              className="w-full h-full object-cover"
                              onError={(e) => {
                                console.error(`Failed to load image for ${internship.title}: ${internship.image}`);
                                e.target.src = `/images/p1.jpeg`;
                              }}
                            />
                          </div>
                          <div className="md:w-1/2 p-6 flex flex-col justify-center">
              ...
              {filteredInternships
                ...
                .map((internship, index) => (
                  ...
                  >
                    <div className="h-48 overflow-hidden">
                      <img
                        src={internship.image ? `/assets/images/${internship.image}` : `/images/p1.jpeg`}
                        alt={internship.title}
                        className="w-full h-full object-cover transition-transform duration-500 hover:scale-110"
                        onError={(e) => {
                          console.error(`Failed to load image for ${internship.title}: ${internship.image}`);
